...
def generate_3d_integral_preds_tensor(heatmaps, num_joints, x_dim, y_dim, z_dim):
    ...
    # accu_x = accu_x * comm.broadcast(torch.arange(float(x_dim)), devices=[accu_x.device.index])[0]
    # accu_y = accu_y * comm.broadcast(torch.arange(float(y_dim)), devices=[accu_y.device.index])[0]
    # accu_z = accu_z * comm.broadcast(torch.arange(float(z_dim)), devices=[accu_z.device.index])[0]
